...
import { mostrarErrorValidacion,mostrarConfirmacionEliminar, mostrarErrorServidor } from './alertaClientes.js';
import { showAlert, showToast, toastInfo } from '../funciones/alertas';
import { eliminarCliente } from '../clientes/crudClientes.js';

export function configurarEventosInformacion(clienteId, direccionId, datosOriginales) {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        // Validar cambios
        if (!detectarCambiosCliente(datosOriginales)) {
        ...
        const [{ error: errorCliente }, { error: errorDireccion }] = await Promise.all([
            supabase.from('clientes').update(clienteUpdate).eq('id', clienteId),
            supabase.from('direcciones').update(direccionUpdate).eq('id', direccionId),
        ]);

        if (errorCliente || errorDireccion) {
            throw new Error((errorCliente || errorDireccion).message);
        }
    ...
    document.getElementById('btn-eliminar').addEventListener('click', () => {
        mostrarConfirmacionEliminar(async () => {
            try {
            await eliminarCliente(clienteId, direccionId);
   ...
   function detectarCambiosCliente(datosOriginales) {
        const direccionSeparada = separarDireccion(document.getElementById('direccion_completa').value);
    ...
    function mostrarConfirmacionEdicion(callback) {
        showAlert({
            ...
            message: 'Se han detectado cambios en el cliente. ¿Deseas guardarlos?',
            buttons: [
                { 
                    text: 'Guardar', 
                    type: 'primary', 
